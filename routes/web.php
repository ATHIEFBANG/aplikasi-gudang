...
use App\Models\CombatTrip; // <-- Tambahkan ini untuk memanggil model
...
// --- HALAMAN UI DRIVER (HANYA INI YANG ADA DI WEB.PHP) ---
Route::get('/track/{token}', function ($token) {
    // Cari data perjalanan beserta detail unit COMBAT berdasarkan token
    $trip = CombatTrip::with('combat')->where('tracking_token', $token)->first();

    // Jika token tidak ditemukan, tampilkan halaman 404
    if (!$trip) {
        abort(404, 'Link tracking tidak valid atau sudah kadaluarsa.');
    }

    // Render file React: resources/js/Pages/Track/DrivePage.jsx
    return Inertia::render('Track/DrivePage', [
        'trip' => $trip
    ]);
})->name('track.driver');

// (CATATAN: Rute /api/track/... untuk start, ping, dan complete JANGAN ditaruh di sini. 
// Biarkan saja di routes/api.php seperti yang sudah kita buat sebelumnya).
