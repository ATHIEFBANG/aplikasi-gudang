...
class DataManagementController extends Controller
{
    ...
    // ==========================================
    // 🟢 SYNC BULK RPM (DARI GOOGLE APPS SCRIPT)
    // ==========================================

    public function syncBulkRpm(Request $request)
    {
        try {
            ...
            if (empty($records) || !is_array($records)) {
                return response()->json([
                    'success' => false,
                    'message' => 'Data records tidak valid atau kosong.'
                ], 400);
            }
            ...
            foreach ($records as $item) {
                // Fleksibel mendukung nama property baik 'rpm_id' atau 'id'
                $rpmId = $item['rpm_id'] ?? $item['id'] ?? null;
                ...
                $dataToUpsert[] = [
                    'rpm_id'          => $this->nullableString($rpmId),
                    'site_id'         => $this->nullableString($item['site_id'] ?? $item['siteid'] ?? null),
                    'rtp'             => $this->nullableString($item['rtp'] ?? null),
                    'mitra'           => $this->nullableString($item['mitra'] ?? null),
                    'bulan'           => $this->nullableString($item['bulan'] ?? null),
                    'tahun'           => $this->nullableString($item['tahun'] ?? null),
                    'approve'         => $this->nullableString($item['approve'] ?? null),
                    'tanggal_submit'  => $this->nullableString($item['tanggal_submit'] ?? $item['tanggalsubmit'] ?? null),
                    'tanggal_approve' => $this->nullableString($item['tanggal_approve'] ?? $item['tanggalapprove'] ?? null),
                    'created_at'      => $now,
                    'updated_at'      => $now,
                ];
            }
            ...
            // Melakukan UPSERT ke tabel 'rpm_masters'
            DB::table('rpm_masters')->upsert(
                $dataToUpsert,
                ['rpm_id'], // Conflict target (Primary Key / Unique Constraint)
                ['site_id', 'rtp', 'mitra', 'bulan', 'tahun', 'approve', 'tanggal_submit', 'tanggal_approve', 'updated_at']
            );

            return response()->json([
                'success' => true,
                'message' => count($dataToUpsert) . ' data RPM berhasil disinkronkan!'
            ], 200);

        } catch (\Exception $e) {
            Log::error("Sync Bulk RPM Error: " . $e->getMessage());

            return response()->json([
                'success' => false,
                'message' => 'Gagal sinkronisasi data RPM: ' . $e->getMessage()
            ], 500);
        }
    }
