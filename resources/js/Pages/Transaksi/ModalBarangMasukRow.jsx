...
import { Trash2, Check, Minus, Plus, Sparkles, Layers, AlertTriangle } from 'lucide-react';
...
export default function ModalBarangKeluarRow({
    ...
    const kondisiStandarList = [
        { id: 'Baru', label: 'Baru (Segel)', icon: Sparkles, color: 'text-emerald-500' },
        { id: 'Bekas', label: 'Bekas (Layak Pakai)', icon: Layers, color: 'text-amber-500' },
        { id: 'Rusak', label: 'Rusak (Afkir)', icon: AlertTriangle, color: 'text-rose-500' },
    ];
    ...
    return (
        <div className="p-4 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/60 relative group space-y-3 transition-all">
            ...
            {/* 4. Pemilih Kondisi untuk Barang Standar (Non-SN) */}
            {!isWajibSn && targetBarang && (
                <div className="p-3 rounded-xl bg-white dark:bg-slate-900/80 border border-slate-200 dark:border-slate-700 space-y-1.5">
                    <div className="flex items-center justify-between">
                        <Label className="text-[11px] font-bold text-slate-700 dark:text-slate-300 flex items-center gap-1.5">
                            <span>Pilih Kondisi Fisik Barang Keluar *</span>
                            <span className="text-[10px] text-slate-400 font-normal">(Barang Standar Tanpa SN)</span>
                        </Label>
                        <span className="text-[10px] font-mono font-bold text-rose-600 dark:text-rose-400">
                            Terpilih: {row.kondisi || 'Baru'}
                        </span>
                    </div>
                    <div className="grid grid-cols-3 gap-2">
                        {kondisiStandarList.map(item => {
                            const isSelected = (row.kondisi || 'Baru') === item.id;
                            const Icon = item.icon;
                            return (
                                <button
                                    key={item.id}
                                    type="button"
                                    onClick={() => onFieldChange(rowIdx, 'kondisi', item.id)}
                                    className={`flex items-center justify-between p-2 rounded-lg border text-xs font-semibold transition-all cursor-pointer ${
                                        isSelected
                                            ? 'bg-rose-600/10 border-rose-600/60 text-rose-600 dark:text-rose-400 shadow-2xs'
                                            : 'bg-slate-50 dark:bg-slate-950/60 border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400 hover:border-rose-400'
                                    }`}
                                >
                                    ...
                                    {isSelected && <Check className="w-3 h-3 text-rose-600 dark:text-rose-400 shrink-0" />}
                                </button>
                            );
                        })}
                    </div>
            ...
            {/* 5. Pemilih Serial Number untuk Barang SN */}
