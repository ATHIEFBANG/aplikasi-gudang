...
export default function Error({ status }) {
    ...
    const errorConfigs = {
        404: {
            ...
            icon: FileQuestion,
            badgeColor: 'text-amber-300 bg-amber-950/70 border-amber-500/50',
        ...
        500: {
            ...
            description: 'Terjadi kendala teknis pada server Gudang Pancapilar Laksana. Tim teknis telah menerima log pemberitahuan ini.',
            icon: ServerCrash,
            badgeColor: 'text-blue-300 bg-blue-950/80 border-blue-500/50',
            iconBg: 'bg-blue-950/70 border-blue-500/40 text-blue-400',
        ...
        503: {
            ...
            icon: Wrench,
            badgeColor: 'text-sky-300 bg-sky-950/70 border-sky-500/50',
        ...
        403: {
            ...
            icon: Lock,
            badgeColor: 'text-yellow-300 bg-yellow-950/70 border-yellow-500/50',
            iconBg: 'bg-yellow-950/60 border-yellow-500/40 text-yellow-400',
    ...
    return (
        <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-4 sm:p-6 md:p-10 relative overflow-hidden font-sans selection:bg-blue-600 selection:text-yellow-300">
            <Head title={`${config.title} - Gudang Pancapilar Laksana`} />
            ...
            <Link 
                href="/" 
                className="absolute top-6 left-6 sm:top-8 sm:left-8 z-50 flex items-center gap-3 group transition-transform duration-200 hover:scale-105"
            >
                <div className="p-2.5 bg-blue-600 text-yellow-400 rounded-xl shadow-lg shadow-blue-600/40 border border-blue-400/30">
                ...
                <div>
                    ...
                    <span className="text-[10px] text-yellow-400/90 font-medium tracking-wide">
                        Gudang Management System
                    </span>
                </div>
            </Link>

            {/* AMBIENT LIGHT BACKGROUND (BIRU & KUNING) */}
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[650px] h-[650px] bg-blue-600/15 blur-[160px] rounded-full pointer-events-none animate-glow" />
            <div className="absolute top-1/3 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[350px] h-[350px] bg-yellow-500/10 blur-[130px] rounded-full pointer-events-none" />

            {/* LAPTOP MOCKUP CONTAINER */}
            <div className="w-full max-w-4xl relative z-10 my-auto pt-12 sm:pt-0">
                
                {/* 1. SCREEN FRAME (BEZEL) */}
                <div className="bg-slate-900 border border-blue-900/60 rounded-t-2xl p-2.5 sm:p-3.5 shadow-2xl shadow-blue-950/50 relative z-20">
                    ...
                    <div className="flex justify-center items-center pb-2 pt-0.5">
                        <div className="flex items-center gap-1.5 px-3 py-0.5 rounded-full bg-slate-950 border border-slate-800">
                            ...
                            <div className="w-1.5 h-1.5 rounded-full bg-yellow-400 animate-pulse shadow-xs shadow-yellow-400" />
                        </div>
                    </div>
                    ...
                    {/* ========================================================================= */}
                    <div className="relative w-full min-h-[480px] sm:min-h-[520px] bg-slate-950 rounded-xl overflow-hidden border border-slate-800 flex flex-col justify-between">
                        ...
                        <div className="relative flex-1 py-8 px-6 sm:px-12 flex items-center justify-center overflow-hidden z-10">
                            ...
                            {/* ERROR DIALOG WINDOW */}
                            <div className="w-full max-w-lg bg-slate-900/95 border border-blue-900/40 rounded-2xl shadow-2xl shadow-black/90 backdrop-blur-xl overflow-hidden">
                                ...
                                <div className="bg-slate-950/90 px-4 py-2.5 border-b border-slate-800 flex items-center justify-between">
                                    ...
                                    <span className="text-[10px] text-yellow-400/80 font-mono font-semibold">
                                        {config.techCode}
                                    </span>
                                </div>
                                ...
                                <div className="p-6 sm:p-8 flex flex-col items-center text-center">
                                    ...
                                    <div className="w-full bg-slate-950/90 rounded-xl p-3 border border-slate-800/80 mt-5 text-left font-mono text-[11px] text-slate-400">
                                        <div className="flex items-center justify-between text-slate-500 pb-1.5 mb-2 border-b border-slate-800 text-[10px]">
                                            <span className="flex items-center gap-1 text-yellow-400">
                                                <Terminal className="w-3 h-3 text-blue-400" /> Console Output
                                            </span>
                                            <span className="text-slate-400">STATUS: {status}</span>
                                        </div>
                                        <div className="text-yellow-400/90 font-mono text-[10px]">
                                    ...
                                    <div className="w-full flex flex-col sm:flex-row items-center gap-3 mt-6">
                                        <Button
                                            ...
                                            variant="outline"
                                            className="w-full sm:w-1/2 h-10 bg-slate-950 hover:bg-slate-800 text-slate-200 border-slate-700/80 text-xs rounded-xl flex items-center justify-center gap-2 transition-all cursor-pointer"
                                        >
                                        ...
                                        <Link href="/" className="w-full sm:w-1/2">
                                            <Button className="w-full h-10 bg-blue-600 hover:bg-blue-500 text-white font-medium text-xs rounded-xl shadow-lg shadow-blue-600/30 transition-all flex items-center justify-center gap-2 cursor-pointer">
                                                <Home className="w-3.5 h-3.5 text-yellow-300" />
                                                Ke Halaman Utama
                                            </Button>
                                        </Link>
                                    </div>
                        ...
                        {/* --------------------------------------------------------------------- */}
                        <div className="w-full h-10 bg-slate-900/95 backdrop-blur-md border-t border-slate-800 px-4 flex items-center justify-between text-xs text-slate-300 select-none z-30">
                            
                            {/* Left Side */}
                            <div className="flex items-center gap-2">
                                <Link href="/">
                                    <button className="flex items-center gap-2 px-3 py-1 bg-blue-600 hover:bg-blue-500 text-yellow-300 rounded-lg font-semibold text-xs transition-all shadow-md shadow-blue-600/30 cursor-pointer">
                            ...
                            <div className="flex items-center gap-3 text-[11px] font-mono text-slate-400">
                                <span className="hidden sm:flex items-center gap-1 text-yellow-300 bg-blue-950/80 px-2 py-0.5 rounded border border-blue-700/50">
                                    System Error
                                </span>
                                ...
                                <div className="flex flex-col items-end text-[10px] leading-tight text-slate-200 border-l border-slate-800 pl-2.5">
                                    <span className="font-semibold text-yellow-400">{currentTime || '12:00'}</span>
            ...
            {/* Footer */}
            <p className="text-[11px] text-slate-500 mt-2 relative z-10 text-center font-medium">
                &copy; {new Date().getFullYear()} PT Pancapilar Laksana. All rights reserved.
            </p>
        </div>
    );
}